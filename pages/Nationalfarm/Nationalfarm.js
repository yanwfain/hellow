...
Page({
  ...
  onLoad: function(options) {
    console.log(options)
    let xinnongye = options.shopCode;
    console.log(xinnongye)
    ...
    classList[0].xinnongye = options.shopCode;
    that.setData({
      xinnongye: xinnongye,
      tabList: tabList,
      classList: classList,
...
function getMoreActive(that) {
  ...
  let data = {
    "xinnongye": that.data.classList[0].xinnongye,
...
function locationShop(that) {
  ...
  var url = constantFields.NEARBYST;
  let city, lat, lng;
  if (that.data.classList[1].city == "附近") {
    city = ""
  } else {
  ...
  var data = {
    ...
    "pageSize": pageSize,
    "city": city,
    "typeCode": that.data.classList[0].xinnongye
  };
  console.log(data+"这是附近的店的参数")
  // 发送请求
  httpUtils.postRequest(url, data).then(function(res) {
    if (res.data.body != "" && res.data.body != [] && res.data.body != null) {
      console.log(res.data.body,"这里是初始加载数据");
...
function getBanner(that) {
  ...
  console.log(that.data.xinnongye)
  let city, lat, lng;
  if (that.data.classList[1].city == "附近") {
    city = ""
  } else {
  ...
  var data = {
    ...
    "lat": lat,
    "lng": lng,
    "shopType": that.data.classList[0].xinnongye,
  };
  console.log(data)
  // 发送请求
  httpUtils.postRequest(url, data).then(function(res) {
